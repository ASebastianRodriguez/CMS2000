/*
 ============================================================================
 Name        : 20180621_ServerTCPnClients.c
 Author      : asrc
 Version     : V1.0
 Copyright   : HB
 Description : Servidor TCP para atender varios clientes utilizando select()
               y fd_set.
               Lee los telegramas extendidos que el CMS2000 envía cada 1s en
               forma continua, para luego guardar los datos en crudo en ar-
               chivos de texto ordenados por fecha y hora.
 ============================================================================
 */
/*******************************************************************************
PENDIENTES 20181012_ServerTCPnClients_V2.0:
	- Hacer archivo log de errores de todas las funciones del programa.
 	- Hacer archivo de configuracion .ini.
 	- Elevar el server a DAEMON para que se cargue en forma automática.
 	- Ver de crear un directorio por cliente CMS2000 de los 30.
	- Hacer bloques de debugeo con printf habilitables por configuración. La
	  idea de los mismos sería chequear la consistencia de los datos recibidos
	  con los enviados a la nube.
	- Implementar funciones de conectividad directa a la nube con la librería
	  MindConnect V3.1.0.0.
...
#include "Headers/Errorlog.h" //Manejo de archivo de log de errores del programa.
#include "Headers/Manejoarchconf.h" //Generación del archivo de configuración del programa.
#include "Headers/Manejodatafiles.h" //Manejo de archivos de datos.

#define TRUE 				  1
#define FALSE 				  0
#define PUERTOSERVER 	  55000 //Ver variable "port".
#define TIMEOUT 			  3
#define LONGBUFFERDATOS 	544
...
{
	if (argc<1) //Para ingresar el puerto desde línea de comandos deberia indicarse "argc<2".
		{
			//El único argumento que se pasa vía línea de comando es el puerto
			//TCP lado server a través del cual se realizará la comunicación.
			printf("\nIngresar el comando seguido del <Puerto>.\n\n");
			return EXIT_FAILURE;
		}

	//*******************************
	// Declaración de variables main.
	//*******************************
	int option 						= TRUE;
	int server_socket 				= 0;
	int longaddr 					= 0;
	int new_socket 					= 0;
	int client_socket[MAXCLIENTES] 	= {0};
	int max_clients 				= MAXCLIENTES;
	int activity 					= 0;
	int j 							= 0;
	int valread 					= 0;
	int sd 							= 0;
	int retgetpeername 				= 0;
	int max_sd 						= 0;
	int longbuffer 					= LONGBUFFERDATOS;
	int b 							= 0;
	int c 							= 0;
	...
	//PUERTO, VER ACÁ:
	//**************************************************************************
	//Hasta no realizar la configuración vía archivo ".ini" el puerto será pasado vía "#define PUERTOSERVER 55000".
	//port = atoi(argv[1]);
	//Pongo el número de puerto directamente.
	int port 						= 55000;
	//**************************************************************************

	struct sockaddr_in address;

	struct timeval tv; //Estructura timeout select().
	tv.tv_sec 						= TIMEOUT; //Valor de timeout en segundos, en nuestro caso 3s.
	tv.tv_usec 						= 0; //Valor de timeout en microsegundo, en nuestro caso 0us.

	char buffer[LONGBUFFERDATOS] 	= {0}; //Buffer de datos.

	//Creo el File Descriptor para manejar las conexiones al socket lado server.
	fd_set readfd;

	//Mensaje de recepción que se envía al cliente.
	char *message 					= "Recibido.\n";
	//***********************************
	// Fin declaración de variables main.
	//***********************************

	//**********************************************
	// Declaración de variables GuardarDataFileJSON.
	//**********************************************
    ...
    strftime(timestampnet, sizeof(timestampnet), "%Y%m%d%H%M%S", ts);
	...
	//**************************************************
	// Fin declaración de variables GuardarDataFileJSON.
	//**************************************************
	//**********************************
	// Declaración de variables HeAJson.
	//**********************************
	//Contadores.
	...
	char *arrayhexrcv 				= buffer;
	char *arrayjson 				= malloc(10000);
	...
	char nullchar[] 	= "\0";
	//**************************************
	// Fin declaración de variables HeAJson.
	//**************************************



	//Inicialización de todos los client_socket[i] a 0.
	for (j = 0; j < max_clients; j++)
	{
		client_socket[j] = 0;
	}

	//Creación de un socket lado server.
	...
	//Seteo el socker lado server para permitir múltiples conexiones, esto es sola-
	//mente un buen hábito de programación, aunque podría funcionar sin este if.
	if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&option, sizeof(option)) < 0)
	{
		printf("Falla al setear opciones del socket con la función \"setsockopt()\".\n");
		exit(EXIT_FAILURE);
	}

	//Creo el tipo de socket: TCP e IPv4.
	address.sin_family = AF_INET;
	address.sin_port = htons(port);
	address.sin_addr.s_addr = INADDR_ANY;

	//Bind (enlace) del socket  lado server al puerto ingresado en línea de comandos.
	if (bind(server_socket, (struct sockaddr *)&address, sizeof(address))<0)
	{
		printf("Fallo al enlazar socket con la función \"bind()\".\n");
		exit(EXIT_FAILURE);
	}
	printf("\nEscuchando puerto: %d.\n", port);

	//Se especifican un máximo de 3 conexiones pendientes para el socket lado server.
	if (listen(server_socket, 3) < 0)
	{
		printf("Fallo al escuchar puerto con la función \"listen()\".\n");
		exit(EXIT_FAILURE);
	}

	//Aceptar la conexión entrante.
	longaddr = sizeof(address);
	printf("Esperando conexiones...\n");

	while(TRUE)
	{
		//Limpio la lista de file descriptors.
		FD_ZERO(&readfd);

		//Agrego un nuevo socket lado server a la lista de file descriptors.
		FD_SET(server_socket, &readfd);
		max_sd = server_socket;

		//Voy agregando child sockets a la lista de file descriptors.
		for (j = 0; j < max_clients; j++)
		{
//			printf("client_socket[%j]=%i\n", j, client_socket[j]);
			//Socket descriptor.
			sd = client_socket[j];

			//Si es válido el socket descriptor, agrego un nuevo child socket a la lista de lectura de file descriptors.
			if(sd > 0)
				FD_SET(sd, &readfd);

			//Número más alto de socket descriptor, neccesito esto para la función select().
			if(sd > max_sd)
			{
				max_sd = sd;
//				printf("max_sd=%i\n", max_sd);
		...
		//Espera la actividad de uno de los sockets, timeout=3.
		activity = select(max_sd + 1, &readfd, NULL, NULL, &tv);

		if ((activity < 0) && (errno!=EINTR))
		{
			printf("Fallo al ejecutar la función \"select()\".\n");
		}

		//Testeo lo que pasa con el socket lado server para saber si tengo una conexión entrante.
		if (FD_ISSET(server_socket, &readfd))
		{
			if ((new_socket = accept(server_socket, (struct sockaddr *)&address, (socklen_t*)&longaddr)) < 0)
			{
				printf("Fallo al aceptar conexión con la función \"accept()\".\n");
				exit(EXIT_FAILURE);
			}
			//Informo al usuario del número de socket utilizado para enviar y recibir comandos.
			printf("\nNueva conexión, socket fd_set %d, IP %s, Puerto %d.\n", new_socket, inet_ntoa(address.sin_addr), ntohs(address.sin_port));

			//Envío un nuevo mensaje de recibido correctamente al cliente.
			if (send(new_socket, message, strlen(message), 0) != strlen(message))
			{
				printf("Fallo al enviar mensaje de recibido al cliente con la función \"send()\".\n");
			}
//			printf("Mensaje enviado al cliente correctamente.\n");

			//Agrego un nuevo socket al array de sockets.
			for (j = 0; j < max_clients; j++)
			{
				...
				if (client_socket[j] == 0)
				{
					...
					printf("Agregando a la lista de sockets con el N° %i.\n", j);
					break;
				}
			}
		}

		//Por el contrario, es una operación I/O en algún otro socket.
		for (j = 0; j < max_clients; j++)
		{
			sd = client_socket[j];
			...
			{
				//Verifica si fue para cerrar y lee el mensaje entrante.

//Verificar que el telegrama termine en 0xFFFFFF7F antes de guardar en el HDD.

				if ((valread = read(sd, buffer, longbuffer)) == 0)
				{
					//Alguien se ha desconectado, obtine los detalles e imprime.
					if ((retgetpeername = getpeername(sd , (struct sockaddr*)&address, (socklen_t*)&longaddr)) == -1)
					{
						printf("Fallo al ejecutar la función \"getpeername()\".\n");
						exit(EXIT_FAILURE);
					}
					else
					{
						printf("Host desconectado, IP %s, Puerto %d.\n", inet_ntoa(address.sin_addr), ntohs(address.sin_port));
						printf("Número de cliente: %i\n", sd);
						//Cierra el socket y lo marca como 0 en la lista para reutilizarlo.
						close(sd);
						client_socket[j] = 0;
					}
				}
				//Guarda los datos de la conexión activa en un archivo.
				else
				{
					//Guardo los datos en un archivo en formato hexadecimal.
					//b = GuardarDataFile((char *)buffer, a); //Esta función guarda los datos obtenidos en un archivo con contenido en hexadecimal.
					//c = GuardarDataFileJSON((char *)buffer, longbuffer); //Esta función guarda los datos obtenidos en un archivo con contenido JSON.
				    ...
				    else
				    {
						...
						else if (n > 0 && n < (maxfilebuffer+2))
						{
							...
							else
							{
								...
								**************************************************************************/
								//Timestamp Externo.
								timestampextrcv[0] = timestampnet[0];
								timestampextrcv[1] = timestampnet[1];
								timestampextrcv[2] = timestampnet[2];
								timestampextrcv[3] = timestampnet[3];
								timestampextrcv[4] = '-';
								timestampextrcv[5] = timestampnet[4];
								timestampextrcv[6] = timestampnet[5];
								timestampextrcv[7] = '-';
								timestampextrcv[8] = timestampnet[6];
								timestampextrcv[9] = timestampnet[7];
								timestampextrcv[10] = 'T';
								timestampextrcv[11] = timestampnet[8];
								timestampextrcv[12] = timestampnet[9];
								timestampextrcv[13] = ':';
								timestampextrcv[14] = timestampnet[10];
								timestampextrcv[15] = timestampnet[11];
								timestampextrcv[16] = ':';
								timestampextrcv[17] = timestampnet[12];
								timestampextrcv[18] = timestampnet[13];
...
/*******************************************************************************
//PARA PRUEBAS:
//					printf("\nStatus creación de archivo: %d.\n", b);
//					printf("Cliente: %i\n", sd);

					//Leo los datos desde el archivo.
//					c = LeerDataFile((char *)nombrearchivo);
//					printf("\nStatus lectura de archivo: %d.\n", c);

					// Imprimimos en pantalla el telegrama extendido de 544 bytes
					// proveniente del CMS2000.
					printf("\n\nTELEGRAMA EXTENDIDO DE 544 BYTES:\n");
					printf("aaaa/mm/dd-hh:mm:ss.ss\n\n");
					for(i=0;i<544;i++)
